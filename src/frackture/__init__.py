...
def frackture_encrypt_payload(payload, key):
    ...
    data = json.dumps(serializable_payload, sort_keys=True).encode()
...
def frackture_decrypt_payload(encrypted_payload, key):
    ...
    # Convert numpy types to JSON-serializable types
    serializable_payload = {
        "symbolic": encrypted_payload["data"]["symbolic"],
        "entropy": [float(x) for x in encrypted_payload["data"]["entropy"]]
    }
    expected_signature = hmac.new(key.encode(), 
                                json.dumps(serializable_payload, sort_keys=True).encode(), 
